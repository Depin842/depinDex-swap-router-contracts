...
/// @notice Supports quoting the calculated amounts for exact input swaps. Is specialized for routes containing a mix of V2 and depindex liquidity.
...
interface IMixedRouteQuoterV1 {
    ...
    /// @return v3SqrtPriceX96AfterList List of the sqrt price after the swap for each depindex pool in the path, 0 for v2 pools
    /// @return v3InitializedTicksCrossedList List of the initialized ticks that the swap crossed for each depindex pool in the path, 0 for v2 pools
    /// @return v3SwapGasEstimate The estimate of the gas that the depindex swaps in the path consume
    ...
    ///      crossing both V2 liquidity pairs and depindex pools.
